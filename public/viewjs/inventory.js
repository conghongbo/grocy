...
$('#display_amount,#qu_id').on('keyup change', function (e) {
	...
	if (!productId || Number.isNaN(newAmount)) {
		$('#inventory-change-info')
			.addClass('d-none')
	...
	Grocy.Api.Get('stock/products/' + productId,
		function (productDetails) {
			var productStockAmount =
				Number.parseFloat(productDetails.stock_amount) || 0;

			var containerWeight = 0.0;

			if (productDetails.product.enable_tare_weight_handling == 1) {
				containerWeight =
					Number.parseFloat(productDetails.product.tare_weight) || 0;
			}

			var inventoryChangeInfo = $('#inventory-change-info');

			/*
			 * Keep Grocy's original booking calculation.
			 *
			 * This value is also used by other inventory features,
			 * for example stock label handling.
			 */
			var estimatedBookingAmount =
				(
					newAmount -
					productStockAmount -
					containerWeight
			...
			$("#amount")
				.attr(
					"data-estimated-booking-amount",
					estimatedBookingAmount
				)
				.trigger("change");

			var absoluteBookingAmount =
				Math.abs(
					Number.parseFloat(estimatedBookingAmount)
				);

			var currentStockDisplay =
				productStockAmount.toLocaleString();
			...
			var quantityUnit = __n(
				absoluteBookingAmount,
			...
			inventoryChangeInfo
				.removeClass(
					'd-none text-success text-danger text-muted'
				)
				.empty();
			...
			/*
			 * ---------------------------------------------------------
			 * Invalid tare weight amount
			 * ---------------------------------------------------------
			 */
			if (
				productDetails.product.enable_tare_weight_handling == 1 &&
				newAmount < containerWeight
			) {
				inventoryChangeInfo.addClass('d-none');
			}
			 ...
			 */
				...
				newAmount >
				productStockAmount + containerWeight
			) {
				inventoryChangeInfo
					...
					.html(
						...
						absoluteBookingAmount.toLocaleString() +
						' ' +
						quantityUnit +
						...
						__t(
							'This means %s will be added to stock',
							absoluteBookingAmount.toLocaleString() +
							' ' +
							quantityUnit
						) +
				...
				/*
				 * Keep original Grocy validation behaviour.
				 */
				...
				if (
					Grocy.FeatureFlags
						.GROCY_FEATURE_FLAG_STOCK_LOCATION_TRACKING
				) {
			 ...
			 */
				...
				inventoryChangeInfo
					...
					.html(
						...
						__t(
							'This means %s will be removed from stock',
							absoluteBookingAmount.toLocaleString() +
							' ' +
							quantityUnit
						) +
			...
			/*
			 * Best-before tracking disabled:
			 * Due date must never become required.
			 */
			if (
				!Grocy.FeatureFlags
