...
function SyncProductPickerAction() {
	var productId = Grocy.Components.ProductPicker.GetValue();

	var inputGroup = Grocy.Components.ProductPicker
		.GetInputElement()
		.closest('.input-group');

	var actionButton = inputGroup.find('.dropdown-toggle');
	var removeIcon = actionButton.find('.remove');
	var pulldownIcon = actionButton.find('.pulldown');

	// The action button itself must always stay visible.
	// Only switch the icon inside it.
	actionButton
		.removeClass('d-none')
		.show();

	removeIcon.removeClass('d-none');
	pulldownIcon.removeClass('d-none');

	if (productId) {
		// Product selected -> show X
		pulldownIcon.hide();
		removeIcon.show();
	}
	else {
		// No product selected -> show dropdown arrow
		removeIcon.hide();
		pulldownIcon.show();
	}
}
...
Grocy.Components.ProductPicker.GetPicker().on('change', function () {
	setTimeout(SyncProductPickerAction, 0);
});

// Visible combobox value changed / cleared
Grocy.Components.ProductPicker.GetInputElement().on('change', function () {
	setTimeout(SyncProductPickerAction, 0);
});

// X clicked
$(document).on('click', '#product_id_text_input ~ .dropdown-toggle .remove, .product-combobox + .combobox-container .remove', function () {
	setTimeout(SyncProductPickerAction, 0);
});
